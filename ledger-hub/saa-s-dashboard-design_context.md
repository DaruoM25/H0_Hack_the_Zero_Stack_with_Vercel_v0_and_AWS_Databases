# CONTEXTE TECHNIQUE DU PROJET WATCHMAN

Ce fichier consolide le code source pour analyse architecturale.
...
## FICHIER : saa-s-dashboard-design_context.md
```md
...
## FICHIER : components\ledgerhub\clients-view.tsx
```tsx
"use client"

import { clients, eurCompact } from "@/lib/ledgerhub-data"
import { useLang } from "@/lib/lang-context"
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {clients.map((c) => (
              ...
              >
                <td className="px-5 py-3.5 font-medium text-slate-100">{c.companyName}</td>
...
## FICHIER : components\ledgerhub\dashboard-view.tsx
```tsx
"use client"
...
function KpiCard({
  ...
  return (
    <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-5">
      ...
      <p className="mt-4 text-2xl font-semibold tracking-tight text-slate-50">{value}</p>
      {trend && (
        <p
...
## FICHIER : components\ledgerhub\header.tsx
```tsx
"use client"
...
## FICHIER : components\ledgerhub\invoice-form.tsx
```tsx
"use client"
...
export function InvoiceForm({ onBack }: { onBack: () => void }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Simulate an async API call to persist the invoice on AWS.
    setTimeout(() => {
          ...
          <div className="sticky top-6 space-y-5 rounded-xl border border-slate-800/60 bg-slate-900/40 p-5">
            ...
            <dl className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <dd className="font-medium text-slate-100">{eur.format(totals.totalHT)}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-slate-400">{f.totalVAT}</dt>
                <dd className="font-medium text-slate-100">{eur.format(totals.totalTVA)}</dd>
              </div>
              <div className="flex items-center justify-between border-t border-slate-800 pt-3">
                <dt className="text-base font-semibold text-slate-50">{f.totalTTC}</dt>
                <dd className="text-base font-semibold text-emerald-400">{eur.format(totals.totalTTC)}</dd>
              </div>
            </dl>
...
## FICHIER : components\ledgerhub\revenue-chart.tsx
```tsx
"use client"
...
## FICHIER : components\ledgerhub\sidebar.tsx
```tsx
"use client"
...
## FICHIER : components\ledgerhub\tax-view.tsx
```tsx
"use client"
